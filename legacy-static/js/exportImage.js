...
function copyToCanvas2d(sourceCanvas, maxWidth) {
 ...
 */
export function compositeCanvas(mapA, mapB, sliderFraction, targetWidth) {
...
function clampFont(size, min, max) {
...
function drawReadoutBox(ctx, label, value, x, bottomY, align, fontSize) {
...
function drawReadoutLine(ctx, text, centerX, bottomY, fontSize) {
 ...
 */
export function drawOverlayLabels(canvas, { before, after, attribution, side = "both" } = {}) {
 ...
 * burns info readouts into the exported image. `maxWidth`, if
 * smaller than the native canvas, downscales the output. `quality` (0-1)
 * controls JPEG compression (ignored for PNG).
 */
export async function exportCompareImage({
  mapA, mapB, sliderFraction, format = "png", target = "slide", filename, labels, maxWidth, quality = 0.92
}) {
