...
interface CdseAttribute {
  Name: string;
  Value: unknown;
}

interface CdseProductWithAttributes extends CdseProduct {
  Attributes?: CdseAttribute[];
}
...
function bboxToWkt([west, south, east, north]: Bbox): string {
...
function tileCodeFromName(name: string): string {
...
async function querySceneList(bbox: Bbox, start: Date, end: Date): Promise<CdseProduct[]> {
...
// cover for picker entries it doesn't need yet.
export async function fetchDayCloudCover(productId: string): Promise<number> {
  try {
    ...
    const product: CdseProductWithAttributes = await res.json();
...
export async function loadSceneData(
  ...
  const byDay = new Map<string, { date: string; productId: string; tiles: Set<string> }>();
  const allTiles = new Set<string>();
  for (const p of products) {
    const day = p.ContentDate.Start.slice(0, 10);
    const tile = tileCodeFromName(p.Name);
    allTiles.add(tile);
    if (!byDay.has(day)) byDay.set(day, { date: day, productId: p.Id, tiles: new Set() });
  ...
  if (priority === "leastcloud") {
    ...
    await Promise.all(
      days.map(async (d) => {
        d.cloudCover = await fetchDayCloudCover(d.productId);
      }),
    );
    ...
    const underCeiling = nearest.find((d) => (d.cloudCover ?? 100) < maxCloud);
